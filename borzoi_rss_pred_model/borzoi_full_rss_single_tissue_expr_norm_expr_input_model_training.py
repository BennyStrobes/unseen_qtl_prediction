...
def parse_args():
	...
	parser.add_argument('--variant-encoder-type', required=False, type=str, default='plain_mlp')
	parser.add_argument('--variant-encoder-architecture', required=False, type=str, default='128,64,32')
	parser.add_argument('--res-mlp-blocks-per-stage', required=False, type=int, default=2)
	parser.add_argument('--res-mlp-dropout-rate', required=False, type=float, default=0.0)
	return parser.parse_args()


def residual_mlp_block(inputs, hidden_dim, l2_reg_variant, dropout_rate, block_name):
	shortcut = inputs
	x = tf.keras.layers.LayerNormalization(name=block_name + '_ln_1')(inputs)
	x = tf.keras.layers.Dense(hidden_dim, kernel_regularizer=l2_reg_variant, name=block_name + '_dense_1')(x)
	x = tf.keras.layers.Lambda(lambda y: tf.nn.gelu(y, approximate=True), name=block_name + '_gelu_1')(x)
	if dropout_rate > 0.0:
		x = tf.keras.layers.Dropout(dropout_rate, name=block_name + '_dropout')(x)
	x = tf.keras.layers.LayerNormalization(name=block_name + '_ln_2')(x)
	x = tf.keras.layers.Dense(inputs.shape[-1], kernel_regularizer=l2_reg_variant, name=block_name + '_dense_2')(x)
	return tf.keras.layers.Add(name=block_name + '_add')([shortcut, x])


def build_variant_encoder(n_borzoi_dimensions, variant_encoder_type, architecture_sizes, l2_reg_variant, res_mlp_blocks_per_stage, res_mlp_dropout_rate):
	borzoi_input = tf.keras.Input(shape=(n_borzoi_dimensions,), name='borzoi_input')
	borzoi_hidden = borzoi_input

	if variant_encoder_type == 'plain_mlp':
	...
	elif variant_encoder_type == 'res_mlp':
		for stage_num, layer_size in enumerate(architecture_sizes, start=1):
			borzoi_hidden = tf.keras.layers.Dense(layer_size, kernel_regularizer=l2_reg_variant, name='borzoi_stage_' + str(stage_num) + '_projection')(borzoi_hidden)
			for block_num in range(res_mlp_blocks_per_stage):
				block_name = 'borzoi_stage_' + str(stage_num) + '_block_' + str(block_num + 1)
				borzoi_hidden = residual_mlp_block(borzoi_hidden, layer_size, l2_reg_variant, res_mlp_dropout_rate, block_name)
	else:
		raise ValueError('Unsupported variant_encoder_type: ' + str(variant_encoder_type))
...
def train_model(train_gene_based_model_data, val_gene_based_model_data, test_index, learning_rate, l2_variant_reg_strength, variant_encoder_type, variant_encoder_architecture, res_mlp_blocks_per_stage, res_mlp_dropout_rate, gene_to_sdev_log_tpm, gene_to_expr_vec, max_epochs=100, use_held_out_genes_for_validation=True):
	...
	variant_encoder = build_variant_encoder(
		n_borzoi_dimensions,
		variant_encoder_type,
		architecture_sizes,
		l2_reg_variant,
...
def main():
	...
	variant_encoder_type = args.variant_encoder_type
	variant_encoder_architecture = args.variant_encoder_architecture
	res_mlp_blocks_per_stage = args.res_mlp_blocks_per_stage
