...
def build_prediction_models(n_borzoi_dimensions, n_expr_gene_dimensions, l2_tissue_reg_strength, l2_variant_reg_strength, variant_encoder_architecture):
	...
	for layer_num, layer_size in enumerate(architecture_sizes, start=1):
		...
		borzoi_hidden = tf.keras.layers.Dense(layer_size, activation=activation_name, kernel_regularizer=l2_reg_variant, name='borzoi_dense_' + str(layer_num))(borzoi_hidden)
	variant_encoder = tf.keras.Model(inputs=borzoi_input, outputs=borzoi_hidden, name='variant_encoder')

	variant_hidden_dim = architecture_sizes[-1]
	film_input = tf.keras.Input(shape=(32,), name='film_input')
	film_hidden = tf.keras.layers.Dense(64, activation='relu', kernel_regularizer=l2_reg_tissue, name='film_dense_1')(film_input)
	film_hidden = tf.keras.layers.Dense(64, activation='relu', kernel_regularizer=l2_reg_tissue, name='film_dense_2')(film_hidden)
	film_gamma = tf.keras.layers.Dense(variant_hidden_dim, activation='linear', kernel_regularizer=l2_reg_tissue, name='film_gamma')(film_hidden)
	film_beta = tf.keras.layers.Dense(variant_hidden_dim, activation='linear', kernel_regularizer=l2_reg_tissue, name='film_beta')(film_hidden)
	film_generator = tf.keras.Model(inputs=film_input, outputs=[film_gamma, film_beta], name='film_generator')

	decoder_input = tf.keras.Input(shape=(variant_hidden_dim,), name='decoder_input')
	decoder_hidden = tf.keras.layers.Dense(max(64, variant_hidden_dim), activation='relu', kernel_regularizer=l2_reg_variant, name='decoder_dense_1')(decoder_input)
	decoder_hidden = tf.keras.layers.Dense(max(32, variant_hidden_dim // 2), activation='relu', kernel_regularizer=l2_reg_variant, name='decoder_dense_2')(decoder_hidden)
	beta_output = tf.keras.layers.Dense(1, activation='linear', kernel_regularizer=l2_reg_variant, name='beta_output')(decoder_hidden)
	beta_decoder = tf.keras.Model(inputs=decoder_input, outputs=beta_output, name='beta_decoder')

	return tissue_encoder, variant_encoder, film_generator, beta_decoder


def compute_beta_mat(tissue_encoder, variant_encoder, film_generator, beta_decoder, gene_borzoi_preds_tf, tissue_expression_tf, training):
	tissue_embeddings_tf = tissue_encoder(tissue_expression_tf, training=training)
	variant_hidden_tf = variant_encoder(gene_borzoi_preds_tf, training=training)
	film_gamma_tf, film_beta_tf = film_generator(tissue_embeddings_tf, training=training)

	modulated_variant_hidden_tf = (
		variant_hidden_tf[:, None, :] * (1.0 + film_gamma_tf[None, :, :]) +
		film_beta_tf[None, :, :]
	)
	modulated_variant_hidden_tf = tf.nn.relu(modulated_variant_hidden_tf)
	n_variants = tf.shape(modulated_variant_hidden_tf)[0]
	n_tissues = tf.shape(modulated_variant_hidden_tf)[1]
	hidden_dim = tf.shape(modulated_variant_hidden_tf)[2]
	flattened_variant_hidden_tf = tf.reshape(modulated_variant_hidden_tf, [-1, hidden_dim])
	flattened_beta = beta_decoder(flattened_variant_hidden_tf, training=training)
	beta_mat = tf.reshape(flattened_beta, [n_variants, n_tissues])
	return 1e-8 * beta_mat
...
def train_model(train_gene_based_model_data, val_gene_based_model_data, gene_expression_data, train_tissue_indices, val_tissue_indices, learning_rate, l2_tissue_reg_strength, l2_variant_reg_strength, variant_encoder_architecture, max_epochs=100, use_held_out_genes_for_validation=True):
	...
	best_film_weights = None
	best_decoder_weights = None
	optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate)

	tissue_encoder, variant_encoder, film_generator, beta_decoder = build_prediction_models(
		n_borzoi_dimensions,
		n_expr_gene_dimensions,
	...
	def compute_observed_and_predicted_gene_zeds(gene_borzoi_preds_tf, gene_zeds_tf, gene_N_eff_tf, gene_LD_tf, tissue_expression_tf, gene_afs_tf, training):
		beta_mat = compute_beta_mat(tissue_encoder, variant_encoder, film_generator, beta_decoder, gene_borzoi_preds_tf, tissue_expression_tf, training=training)
	...
	def compute_gene_loss(gene_borzoi_preds_tf, gene_zeds_tf, gene_N_eff_tf, gene_LD_tf, gene_inv_LD_tf, tissue_expression_tf, gene_afs_tf, training, compute_correlation=False):
		...
		pred_gene_zeds_mat = tf.sqrt(tf.boolean_mask(gene_N_eff_tf, valid_tissues, axis=1)) * tf.boolean_mask(tf.matmul(gene_LD_tf, beta_std_mat), valid_tissues, axis=1)
		residuals = tf.boolean_mask(gene_zeds_tf, valid_tissues, axis=1) - pred_gene_zeds_mat

		loss = tf.reduce_sum(residuals * tf.matmul(gene_inv_LD_tf, residuals))

		if compute_correlation:
			obs = tf.reshape(tf.boolean_mask(gene_zeds_tf, valid_tissues, axis=1), [-1])
	...
	def train_step(gene_borzoi_preds_tf, gene_zeds_tf, gene_N_eff_tf, gene_LD_tf, gene_inv_LD_tf, gene_afs_tf):
		...
		trainable_variables = tissue_encoder.trainable_variables + variant_encoder.trainable_variables + film_generator.trainable_variables + beta_decoder.trainable_variables
	...
	for epoch_iter in range(max_epochs):
		...
		if epoch_val_loss < best_val_loss:
			...
			best_film_weights = film_generator.get_weights()
			best_decoder_weights = beta_decoder.get_weights()
			status = ' best'
	...
	if best_tissue_weights is not None:
		...
		film_generator.set_weights(best_film_weights)
		beta_decoder.set_weights(best_decoder_weights)

	return tissue_encoder, variant_encoder, film_generator, beta_decoder


def evaluate_model(tissue_encoder, variant_encoder, film_generator, beta_decoder, gene_based_model_data, train_gene_based_model_data, val_gene_based_model_data, test_gene_based_model_data, gene_expression_data, test_tissue_index, model_training_output_stem):
	...
	with open(output_file, 'w') as t, open(variant_output_file, 'w') as v:
		...
		for gene_name, gene_snp_summary_file, gene_zed_file, gene_N_eff_file, gene_LD_file, gene_inv_LD_file, gene_borzoi_pred_file, n_gene_snps in gene_based_model_data:
			...
			beta_mat = compute_beta_mat(tissue_encoder, variant_encoder, film_generator, beta_decoder, gene_borzoi_preds_tf, test_tissue_expression_tf, training=False)
			genotype_sd = tf.sqrt(2.0 * gene_afs_tf * (1.0 - gene_afs_tf))
			beta_std_mat = beta_mat * genotype_sd[:, None]
...
def main():
	...
	# Train
	tissue_encoder, variant_encoder, film_generator, beta_decoder = train_model(
	...
	# Evaluate
	evaluate_model(tissue_encoder, variant_encoder, film_generator, beta_decoder, gene_based_model_data, train_gene_based_model_data, val_gene_based_model_data, test_gene_based_model_data, gene_expression_data, test_tissue_index, model_training_output_stem)
	# Save model results
	...
	film_generator.save(model_training_output_stem + '_film_generator.keras')
	beta_decoder.save(model_training_output_stem + '_beta_decoder.keras')
