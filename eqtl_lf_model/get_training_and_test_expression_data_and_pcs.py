...
training_tissue_file = sys.argv[2]
test_tissue_file = sys.argv[3]
training_expression_file = sys.argv[4]
test_expression_file = sys.argv[5]
training_expression_pc_file = sys.argv[6]
test_expression_pc_file = sys.argv[7]
n_pcs = int(sys.argv[8])

# Extract tissue names
training_tissues = extract_tissue_names(training_tissue_file)
test_tissues = extract_tissue_names(test_tissue_file)


# Filter expression to only training tissues or only testing tissues
filter_expression_to_set_of_tissues(training_tissues, processed_all_sample_expression_file, training_expression_file)
filter_expression_to_set_of_tissues(test_tissues, processed_all_sample_expression_file, test_expression_file)


# Extract gene expression data
E_train, samples_train, genes_train = extract_gene_expression_data(training_expression_file)
E_test, samples_test, genes_test = extract_gene_expression_data(test_expression_file)
...
# Test phase
Z_test = pca_test(np.transpose(E_test), pca, gene_means, gene_stds)


# Print PCs to output
print_pcs_to_output(Z_train, samples_train, training_expression_pc_file)
print_pcs_to_output(Z_test, samples_test, test_expression_pc_file)
